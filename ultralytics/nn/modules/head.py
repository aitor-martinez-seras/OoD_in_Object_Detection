...
class Detect(nn.Module):
    ...
    def forward(self, x):
        ...
        one_img_preds = y[0, 4:, :].T.cpu().detach().numpy()
        plot_values = np.zeros((one_img_preds.shape[0], 2))
        # Now for the first dimension compute the max value of the class
        plot_values[:, 0] = np.max(one_img_preds, axis=1)
        # Now for the second dimension compute the entropy of the class probabilities
        #plot_values[:, 1] = -np.sum(one_img_preds * np.log(one_img_preds), axis=1)
        plot_values[:, 1] = entropy(one_img_preds, base=2, axis=1)
        # Now plot the values in a scatter
        plt.figure(figsize=(10, 6))  # Increase figure size
        plt.scatter(plot_values[:, 0], plot_values[:, 1], s=1, alpha=0.5)  # s is the size, alpha is the transparency
        plt.savefig('prueba_entropy.png')
        plt.close()
        # Now make same plot but with Y axis with the value of the difference between the max value and the second max value
        plot_values = np.zeros((one_img_preds.shape[0], 2))
        # Now for the first dimension compute the max value of the class
        plot_values[:, 0] = np.max(one_img_preds, axis=1)
        # Now for the second dimension compute the difference between the max value and the second max value
        plot_values[:, 1] = np.max(one_img_preds, axis=1) - np.partition(one_img_preds, -2, axis=1)[:, -2]
        # Now plot the values in a scatter
        plt.figure(figsize=(10, 6))
        plt.scatter(plot_values[:, 0], plot_values[:, 1], s=1, alpha=0.5)  # Adjust size and transparency
        plt.savefig('prueba_diff.png')
        plt.close()

        # OTROS PLOTS
        from scipy.stats import entropy
        import matplotlib.pyplot as plt
        # Assuming y[0, 4:, :].T.cpu().detach().numpy() gives your sigmoid outputs
        # Replace this with the actual computation or loading of your data
        one_img_preds = y[0, 4:, :].T.cpu().detach().numpy()

        # Condition for splitting based on max value
        above_0_1 = np.max(one_img_preds, axis=1) > 0.1
        below_0_1 = np.max(one_img_preds, axis=1) <= 0.1

        # Calculate entropy
        entropy_values = entropy(one_img_preds, base=2, axis=1)

        # Efficient calculation of differences between max value and second max value
        top2_values = np.partition(one_img_preds, -2, axis=1)[:, -2:]
        differences = top2_values[:, -1] - top2_values[:, -2]

        # Entropy plot for Max > 0.1
        plt.figure(figsize=(10, 6))
        plt.scatter(np.max(one_img_preds[above_0_1], axis=1), entropy_values[above_0_1], s=1, alpha=0.5, color='blue')
        plt.title('Entropy for Max Value > 0.1')
        plt.savefig('entropy_above_0_1.png')
        plt.close()

        # Entropy plot for Max ≤ 0.1
        plt.figure(figsize=(10, 6))
        plt.scatter(np.max(one_img_preds[below_0_1], axis=1), entropy_values[below_0_1], s=1, alpha=0.5, color='red')
        plt.title('Entropy for Max Value ≤ 0.1')
        plt.savefig('entropy_below_0_1.png')
        plt.close()

        # Difference plot for Max > 0.1
        plt.figure(figsize=(10, 6))
        plt.scatter(np.max(one_img_preds[above_0_1], axis=1), differences[above_0_1], s=1, alpha=0.5, color='blue')
        plt.title('Difference for Max Value > 0.1')
        plt.savefig('diff_above_0_1.png')
        plt.close()

        # Difference plot for Max ≤ 0.1
        plt.figure(figsize=(10, 6))
        plt.scatter(np.max(one_img_preds[below_0_1], axis=1), differences[below_0_1], s=1, alpha=0.5, color='red')
        plt.title('Difference for Max Value ≤ 0.1')
        plt.savefig('diff_below_0_1.png')
        plt.close()

        # Plot for Max > 0.1 with Sum of Scores on Y-axis
        # Calculate sum of scores for each prediction
        sum_of_scores = np.sum(one_img_preds, axis=1)
        plt.figure(figsize=(10, 6))
        plt.scatter(np.max(one_img_preds[above_0_1], axis=1), sum_of_scores[above_0_1], s=1, alpha=0.5, color='blue')
        plt.title('Sum of Scores for Max Value > 0.1')
        plt.xlabel('Max Value')
        plt.ylabel('Sum of Scores')
        plt.savefig('sum_scores_above_0_1.png')
        plt.close()

        # Plot for Max ≤ 0.1 with Sum of Scores on Y-axis
        plt.figure(figsize=(10, 6))
        plt.scatter(np.max(one_img_preds[below_0_1], axis=1), sum_of_scores[below_0_1], s=1, alpha=0.5, color='red')
        plt.title('Sum of Scores for Max Value ≤ 0.1')
        plt.xlabel('Max Value')
        plt.ylabel('Sum of Scores')
        plt.savefig('sum_scores_below_0_1.png')
        plt.close()
